...
from .conf import RECEIVER_EMAIL, POSTBACK_ENDPOINT, SANDBOX_POSTBACK_ENDPOINT
...
class PayPalStandardBase(Model):
    ...
    txn_id = models.CharField(
        "Transaction ID", max_length=19, blank=True,
        help_text="PayPal transaction ID.", db_index=True)
    txn_type = models.CharField(
        "Transaction Type", max_length=128, blank=True,
    ...
    receipt_id = models.CharField(
        max_length=64, blank=True)  # 1335-7816-2936-1451
    ...
    def verify(self, item_check_callable=None):
        ...
        Checks for obvious signs of weirdness in the payment and flags
        appropriately.

        Provide a callable that takes an instance of this class as a parameter
        and returns a tuple (False, None) if the item is valid. Should return
        (True, "reason") if the item isn't valid. Strange but backward
        compatible :) This function should check that `mc_gross`,
        `mc_currency` `item_name` and `item_number` are all correct.
    ...
    def _verify_postback(self):
        """
        Check self.response is valid and call self.set_flag if there is an
        error.
        """
        raise NotImplementedError
